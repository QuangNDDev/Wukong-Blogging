function ImageUpload({
  ...
  return (
    <label
      className={`cursor-pointer flex items-center justify-center bg-grayLight border border-dashed w-full min-h-[200px] rounded-lg ${clasName} relative overflow-hidden group`}
    >
      <input
        type="file"
        name={name}
      ...
      />
      {progress !== 0 && (
        <div className="absolute z-10 w-16 h-16 border-8 border-green-500 rounded-full loading border-t-transparent animate-spin" />
      )}
      {image ? (
        <>
